...
export default async function AddSalePage({ searchParams }: AddSalePageProps) {
  ...
  return <main className="review-page catalogue-page">
    <header className="site-header"><Link className="brand" href="/" aria-label="RAR Index home"><span className="brand-mark">R</span><span>RAR</span><em>Index</em></Link><Link className="header-note" href="/review">Review queue -&gt;</Link><Link className="header-note" href="/price-import">CSV batch import -&gt;</Link></header>
    ...
    <section className="catalogue-content"><div className="section-intro"><p className="eyebrow">The simple route</p><h2>Capture the evidence, then decide</h2><p className="section-copy">Use this for one sale at a time. The exact RAR edition and the completed-listings check are still required; that is what keeps a convenient form from quietly polluting the index.</p></div><QuickSaleForm initialEditionId={initialEditionId} /><section className="catalogue-rules" aria-label="Quick sale safeguards"><div><span>1</span><strong>Exact record</strong><p>Choose the published RAR edition, not a similar title.</p></div><div><span>2</span><strong>Original source</strong><p>Save the source link and marketplace ID so anyone can inspect it.</p></div><div><span>3</span><strong>Human review</strong><p>The queue decides whether it becomes market evidence.</p></div></section></section>
  </main>;
}
