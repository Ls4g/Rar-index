...
export default async function AgentControlPage() {
  ...
  const [systemResult, controlsResult, runsResult, actionsResult, unlabelledResult, ruleDashboardResult, autopilotResult] = await Promise.all([
    ...
    readUnlabelledScoutDecisionQueue(admin).then((data) => ({ data, error: null })).catch((error: Error) => ({ data: [], error })),
  ...
  return (
    <main className="review-page catalogue-page agent-control-page">
      ...
      {setupError ? <section className="catalogue-content"><div className="review-empty"><strong>The autonomy database is not ready.</strong><p>Apply the 20260817 agent control-plane migration, then reload this page. {setupError.message}</p></div></section> : <AgentControlCentre actions={(actionsResult.data ?? []) as never[]} autopilot={autopilotResult.data as never} controls={(controlsResult.data ?? []) as never[]} globalPaused={Boolean(systemResult.data?.global_paused)} historicalDecisions={(unlabelledResult.data ?? []) as never[]} pauseReason={systemResult.data?.pause_reason ?? null} ruleDashboard={ruleDashboardResult.data as never} runs={(runsResult.data ?? []) as never[]} />}
    </main>
  );
}
