...
export default async function AgentControlPage() {
  ...
  const [systemResult, controlsResult, runsResult, actionsResult, autopilotResult, ebayHealth, scoutPriorityResult] = await Promise.all([
  ...
  return (
    <main className="review-page catalogue-page agent-control-page">
      ...
      {setupError ? <section className="catalogue-content"><div className="review-empty"><strong>The autonomy database is not ready.</strong><p>Apply the 20260817 agent control-plane migration, then reload this page. {setupError.message}</p></div></section> : <AgentControlCentre actions={(actionsResult.data ?? []) as never[]} autopilot={autopilotResult.data as never} controls={(controlsResult.data ?? []) as never[]} ebayHealth={ebayHealth} globalPaused={Boolean(systemResult.data?.global_paused)} pauseReason={systemResult.data?.pause_reason ?? null} runs={(runsResult.data ?? []) as never[]} scoutPriorityCount={scoutPriorityResult.data} />}
    </main>
  );
}
