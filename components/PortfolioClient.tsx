...
export default function PortfolioClient({ initialEditionId = "" }: { initialEditionId?: string }) {
  ...
  return <main className="portfolio-page">
    ...
    {!userEmail ? <section className="portfolio-auth"><div><p className="eyebrow">RAR Portfolio</p><h1>Know what you own.</h1><p>Private holdings, linked to RAR&apos;s edition records and the market evidence behind them.</p>{initialEditionId ? <p className="portfolio-add-context">You&apos;re adding a specific RAR edition. Sign in or create a free account and it will be ready to add.</p> : <p className="portfolio-add-context">Create a free account to keep your holdings private and available across devices.</p>}</div><form className="portfolio-auth-form" onSubmit={submitAuth}><p className="eyebrow">{mode === "sign-in" ? "Sign in" : "Create account"}</p><label>Email<input type="email" required value={email} onChange={(event) => setEmail(event.target.value)} autoComplete="email" /></label><label>Password<input type="password" required minLength={6} value={password} onChange={(event) => setPassword(event.target.value)} autoComplete={mode === "sign-in" ? "current-password" : "new-password"} /></label><button type="submit">{mode === "sign-in" ? "Sign in" : "Create account"}</button><button className="portfolio-text-button" type="button" onClick={() => { setMode(mode === "sign-in" ? "sign-up" : "sign-in"); setAuthMessage(""); }}>{mode === "sign-in" ? "Need an account? Create one" : "Already have an account? Sign in"}</button>{authMessage ? <p role="status">{authMessage}</p> : null}</form></section> : <>
