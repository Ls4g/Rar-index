...
export default function BrowseEditions({ editions }: { editions: BrowseEdition[] }) {
  ...
  const results = useMemo(() => {
    ...
    return editions.filter((edition) => {
      ...
      return (!term || searchable.includes(term)) && (language === "all" || edition.language === language) && (publisher === "all" || edition.publisher === publisher) && (!firstPrintOnly || edition.printing_number === 1);
    });
  }, [editions, firstPrintOnly, language, publisher, query]);
  ...
  return <>
    ...
    {results.length ? <div className="browse-grid">{results.map((edition) => <Link href={`/edition/${edition.id}`} className="browse-card" key={edition.id}>
      <p>{[edition.series, edition.volume_number ? `Vol. ${edition.volume_number}` : null, edition.language].filter(Boolean).join(" · ")}</p>
